<script lang="ts">
    ...
    let positionUniforms: any = null;
    let velocityUniforms: any = null;
    let birdUniforms: any = null;
    let mouseX = 0;
    let mouseY = 0;
    let windowHalfX = 0;
    let windowHalfY = 0;
    ...
    function render() {
        if (!renderer || !camera || !scene || !gpuCompute) return;
...
<div bind:this={container} class="birds-container w-full h-full"></div>
